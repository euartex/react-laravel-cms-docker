...
import AssetEditForm from "./ArticleEditForm.js";
...
import { formatDateMMDDYYYYwithTime } from "helpers/formatHelper";
...
const AssetsPage = ({ allowEdit }) => {
  ...
  const getPage = () => {
    axiosInstance
      ...
      .catch(error => {
        ...
        dispatch({ type: REQUEST_SUCCESS, payload: { data: mockedAssets } });
        return notificationRef?.current?.showNotification();
      });
  };
  ...
  //TODO: check if handleVideo needed at all here
  const handleVideoUpload = ({ target }) => {
    ...
    axiosInstance.post(
      `/articles/${form.id}/mezanine/upload/video`,
      data,
      config
    );
  };
  ...
  const onDeleteRows = () => {
    axiosInstance
      .delete("/articles", { data: { ids: selectedRows.map(row => row.id) } })
      .then(() => {
        dispatch({
          type: SET_MESSAGE,
          payload: {
            messageStatus: "success",
            message: "Selected rows were deleted"
          }
        });
        setSelectedRows([]);
        // setTimeout(() => {
  ...
  const handleDelete = () => {
    ...
    axiosInstance
      .delete("/articles", { data: { ids: [idToDelete] } })
      .then(() => {
        dispatch({
          type: SET_MESSAGE,
          payload: {
  ...
  const submitForm = () => {
    ...
    const handleFormSubmitResponse = () => {
      dispatch({
        ...
        payload: MESSAGES.updated("Asset")
      });
    ...
    if (!savedForm) {
      ...
      axiosInstance
        .post(
          "articles",
          convertToFormData({ type: "article", ...fieldsToSend })
        )
      ...
      axiosInstance
        .post(`articles/${savedForm.id}`, convertToFormData(changedFields))
        .then(handleFormSubmitResponse)
        .catch(handleFormSubmitError);
    }
  };
          ...
          {formIsOpened && (
            <Fade in={formIsOpened}>
              <AssetEditForm
                ...
                companyOptions={convertDataToOptions(companyOptions)}
              />
            </Fade>
          )}
          {confirmIsOpened && (
